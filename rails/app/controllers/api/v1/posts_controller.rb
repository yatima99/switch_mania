class Api::V1::PostsController < Api::V1::BaseController
  ...
  def index
    ...
    posts = posts.includes([:tags, :user, comments: [:user]]).order(created_at: :desc).page(params[:page] || 1).per(12)

    render json: posts, meta: pagination(posts), adapter: :json
  end

  def show
    post = Post.published.includes(:tags, comments: [:user]).find(params[:id])
